        ...
        match plan {
            ...
            LogicalPlan::Limit(ref limit) => {
                ...
                match (self.exists_sub_query, plan_hold_outer) {
                    (false, true) => {
                        if is_scalar_aggregate(limit.input.as_ref()) {
                            Ok(Transformed::no(plan))
                        } else {
                            self.can_pull_up = false;
                            Ok(Transformed::new(plan, false, TreeNodeRecursion::Jump))
...
fn is_scalar_aggregate(plan: &LogicalPlan) -> bool {
    match plan {
        LogicalPlan::Aggregate(agg) => agg.group_expr.is_empty(),
        LogicalPlan::Projection(proj) => is_scalar_aggregate(proj.input.as_ref()),
        LogicalPlan::Filter(filter) => is_scalar_aggregate(filter.input.as_ref()),
        _ => false,
    }
}
