...
impl ConcurrentPlanExecutor {
    ...
    fn export_to_json(&self, data: &Table, path: &str) -> Result<Table> {
        ...
        for row_idx in 0..n {
            ...
            for (i, field) in schema.fields().iter().enumerate() {
                let val = columns[i].get_value(row_idx);
    ...
    fn export_to_csv(&self, data: &Table, path: &str, options: &ExportOptions) -> Result<Table> {
        ...
        for row_idx in 0..n {
            let values: Vec<String> = columns
                .iter()
                .map(|col| Self::value_to_csv_string(&col.get_value(row_idx)))
                .collect();
    ...
    fn build_arrow_arrays(data: &Table, schema: &Schema, num_rows: usize) -> Result<Vec<ArrayRef>> {
            ...
            let array: ArrayRef = match &field.data_type {
                ...
                DataType::Int64 => {
                    ...
                    match column {
                        ...
                        Some(Column::Float64 { data, nulls }) => {
                            for (i, &val) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                        ...
                        Some(Column::Int64 { data, nulls }) => {
                            for (i, &val) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    ...
                                    builder.append_value(val as f64);
                                }
                            }
                        }
                        _ => {
                ...
                DataType::DateTime | DataType::Timestamp => {
                    ...
                    match column {
                        Some(Column::DateTime { data, nulls }) => {
                        ...
                        Some(Column::Timestamp { data, nulls }) => {
                            for (i, ts) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                ...
                | DataType::Range(_) => {
                    ...
                    match column {
                        ...
                        Some(Column::Numeric { data, nulls }) => {
                        ...
                        Some(Column::Bytes { data, nulls }) => {
                            for (i, b) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                                    builder.append_value(hex::encode(b));
                                }
                            }
                        }
                        Some(Column::Time { data, nulls }) => {
                            for (i, t) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                                    builder.append_value(t.to_string());
                                }
                            }
                        }
                        Some(Column::Json { data, nulls }) => {
                            for (i, j) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                                    builder.append_value(j.to_string());
                                }
                            }
                        }
                        Some(Column::Array { data, nulls, .. }) => {
                            for (i, a) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                                    builder.append_value(format!("{:?}", a));
                                }
                            }
                        }
                        Some(Column::Struct { data, nulls, .. }) => {
                            for (i, s) in data.iter().enumerate() {
                                if nulls.is_null(i) {
                                    builder.append_null();
                                } else {
                                    builder.append_value(format!("{:?}", s));
                                }
                            }
                        }
                        Some(col) => {
                            for i in 0..num_rows {
                                if col.is_null(i) {
                                    ...
                                    builder.append_value(Self::value_to_csv_string(&val));
                                }
                            }
                        }
                        None => {
