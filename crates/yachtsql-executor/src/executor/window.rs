...
pub(crate) fn partition_rows_columnar(
    n: usize,
    columns: &[&Column],
    partition_by: &[Expr],
    evaluator: &ValueEvaluator,
...
pub(crate) fn sort_partition_columnar(
    columns: &[&Column],
    indices: &mut [usize],
    order_by: &[SortExpr],
    evaluator: &ValueEvaluator,
    ...
    match func_type {
        WindowFuncType::Window(func) => match func {
            ...
            WindowFunction::Ntile => {
                ...
                let base_size = partition_size / n_val;
                let extra = partition_size % n_val;
                let mut bucket = 1usize;
                let mut in_bucket = 0usize;
                for _ in 0..partition_size {
                    let bucket_size = if bucket <= extra {
                    ...
                    results.push(Value::Int64(bucket as i64));
                    in_bucket += 1;
                    if in_bucket >= bucket_size && bucket < n_val {
                        bucket += 1;
                        in_bucket = 0;
                    }
                }
        ...
        WindowFuncType::Aggregate(func) => {
            ...
            if let Some(frame) = frame {
                for curr_pos in 0..partition_size {
                    ...
                    let agg_result =
                        compute_aggregate_columnar(func, expr, columns, &frame_indices, evaluator)?;
                    results.push(agg_result);
                }
            } else if has_order_by {
                let mut peer_groups: Vec<(usize, usize)> = Vec::new();
                ...
                let agg_result =
                    compute_aggregate_columnar(func, expr, columns, sorted_indices, evaluator)?;
                results = vec![agg_result; partition_size];
            }
        }
    }
