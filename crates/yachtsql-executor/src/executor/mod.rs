...
impl<'a> PlanExecutor<'a> {
    ...
    pub(crate) fn execute_plan(&mut self, plan: &PhysicalPlan) -> Result<Table> {
        match plan {
    ...
    fn execute_gap_fill(
        ...
        let evaluator = ValueEvaluator::new(storage_schema)
            .with_variables(&self.variables)
            .with_system_variables(self.session.system_variables())
            .with_user_functions(&self.user_function_defs);
        let empty_record = Record::new();
        ...
        for row_idx in 0..n {
            ...
            let partition_key: Vec<Value> = partition_indices
                .iter()
                .map(|&idx| columns[idx].get_value(row_idx))
                .collect();

            let values_for_row: Vec<Value> = value_col_indices
                .iter()
                .map(|(idx, _)| columns[*idx].get_value(row_idx))
                .collect();
