...
impl<'a> PlanExecutor<'a> {
    pub(crate) fn execute_merge(
        ...
        let target_evaluator = ValueEvaluator::new(&target_schema);
        let source_evaluator = ValueEvaluator::new(&source_schema);
        ...
        let target_rows: Vec<Vec<Value>> = (0..target_n)
            ...
            .collect();
        let source_rows: Vec<Vec<Value>> = (0..source_n)
            .map(|i| source_columns.iter().map(|c| c.get_value(i)).collect())
            .collect();
    ...
    fn process_matched_target_rows(
        ...
        target_evaluator: &ValueEvaluator,
        on: &Expr,
        clauses: &[MergeClause],
                    ...
                    MergeClause::MatchedUpdate {
                        ...
                        if condition_matches {
                            ...
                            for assignment in assignments {
                                ...
                                {
                                    let new_val = if Self::expr_contains_subquery(&assignment.value)
                                    {
                                        self.eval_expr_with_subquery(
                                            &assignment.value,
                                            combined_schema,
    ...
    fn process_unmatched_source_rows(
        ...
        source_matched: &[bool],
        target_schema: &Schema,
        source_evaluator: &ValueEvaluator,
        clauses: &[MergeClause],
