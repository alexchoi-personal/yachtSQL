...
impl<'a> PlanExecutor<'a> {
    pub fn execute_delete(
        ...
        if has_subquery {
            ...
            let new_table = table.gather_rows(&keep_indices);
            ...
            match &resolved_filter {
                Some(expr) => {
                    let evaluator = ColumnarEvaluator::new(&base_schema)
                        .with_variables(&self.variables)
                        .with_system_variables(self.session.system_variables())
                        .with_user_functions(&self.user_function_defs);

                    let delete_mask = evaluator.evaluate(expr, &table)?;
                    let mut keep_indices = Vec::new();
                    for i in 0..n {
                        let should_delete = delete_mask.get_value(i).as_bool().unwrap_or(false);
                        if !should_delete {
                            keep_indices.push(i);
                        }
                    }
                ...
                None => {
                    let new_table = Table::empty(base_schema.clone());
                    self.catalog.replace_table(table_name, new_table)?;
                }
            }
        }
    ...
    pub(super) fn table_column_to_expr_list(&self, table: &Table) -> Result<Vec<Expr>> {
        ...
        let n = table.row_count();
        let first_col = table.columns().iter().next().map(|(_, c)| c);
