...
impl<'a> PlanExecutor<'a> {
    pub(crate) fn execute_update(
        ...
        let evaluator_for_defaults = ValueEvaluator::new(&target_schema);
    ...
    fn execute_update_with_from(
        ...
        for target_idx in 0..target_n {
            let target_row: Vec<Value> = target_columns
                .iter()
                .map(|c| c.get_value(target_idx))
                .collect();

            for from_idx in 0..from_n {
                let from_row: Vec<Value> =
                    from_columns.iter().map(|c| c.get_value(from_idx)).collect();

                let mut combined_values = target_row.clone();
                combined_values.extend(from_row);
        ...
        for idx in 0..target_n {
            if let Some(updated_row) = updated_rows.get(&idx) {
                new_table.push_row(updated_row.clone())?;
            } else {
                let row: Vec<Value> = target_columns.iter().map(|c| c.get_value(idx)).collect();
    ...
    fn execute_update_without_from(
        ...
        let update_mask: Vec<bool> = if let Some(filter_expr) = filter {
            if has_subquery {
                ...
                for i in 0..n {
                    let row_values: Vec<Value> = columns.iter().map(|c| c.get_value(i)).collect();
                    let record = Record::from_values(row_values);
                    let should_update = self
                        .eval_expr_with_subquery(filter_expr, &target_schema_with_source, &record)?
                        .as_bool()
                        .unwrap_or(false);
                    mask.push(should_update);
                }
                mask
            } else {
                let col_evaluator = ColumnarEvaluator::new(target_schema)
                    .with_variables(&self.variables)
                    .with_system_variables(self.session.system_variables())
                    .with_user_functions(&self.user_function_defs);
                let mask_col = col_evaluator.evaluate(filter_expr, table)?;
                (0..n)
                    .map(|i| mask_col.get_value(i).as_bool().unwrap_or(false))
                    .collect()
            }
        } else {
            vec![true; n]
        ...
        for (i, &should_update) in update_mask.iter().enumerate() {
            let row_values: Vec<Value> = columns.iter().map(|c| c.get_value(i)).collect();
