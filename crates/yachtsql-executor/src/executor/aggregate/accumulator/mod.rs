...
pub(crate) use basic::{
    AvgAccumulator, CountAccumulator, FirstAccumulator, LastAccumulator, MaxAccumulator,
    MinAccumulator, SumAccumulator,
};
pub(crate) use bitwise::{BitAndAccumulator, BitOrAccumulator, BitXorAccumulator};
pub(crate) use collection::{ArrayAggAccumulator, CountDistinctAccumulator, StringAggAccumulator};
pub(crate) use conditional::{
    AvgIfAccumulator, CountIfAccumulator, MaxIfAccumulator, MinIfAccumulator, SumIfAccumulator,
};
...
pub(crate) use utils::{
    ...
    has_ignore_nulls, is_distinct_aggregate,
};
...
impl Accumulator {
    ...
    pub(crate) fn accumulate(&mut self, value: &Value) -> Result<()> {
        match self {
            ...
            Accumulator::Sum(sum) => {
                if let Some(v) = utils::value_to_f64(value) {
            ...
            Accumulator::Variance {
                ...
                if let Some(x) = utils::value_to_f64(value) {
                    *count += 1;
                    let delta = x - *mean;
    ...
    pub(crate) fn accumulate_bivariate(&mut self, x: &Value, y: &Value) -> Result<()> {
        let x_val = utils::value_to_f64(x);
        let y_val = utils::value_to_f64(y);
