...
impl<'a> Parser<'a> {
    ...
    pub fn parse_statement(&mut self) -> Result<Statement, ParserError> {
        ...
        match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::LOOP if self.dialect.supports_loop_end_loop() => self.parse_loop(None),
                Keyword::FOR if self.dialect.supports_loop_end_loop() => self.parse_for_statement(),
            ...
            if self.parse_keywords(&[Keyword::GROUPING, Keyword::SETS]) {
                ...
                let result = self.parse_comma_separated(|p| {
                    if p.parse_keyword(Keyword::ROLLUP) {
                        ...
                        let rollup_exprs =
                            p.parse_comma_separated(|p2| p2.parse_tuple(true, true))?;
                        p.expect_token(&Token::RParen)?;
    ...
    fn parse_create_function_name_and_params(
        ...
        let parse_function_param =
            |parser: &mut Parser| -> Result<OperateFunctionArg, ParserError> {
                ...
                let default_expr =
                    if parser.consume_token(&Token::Eq) || parser.parse_keyword(Keyword::DEFAULT) {
    ...
    pub fn parse_create_view(
        ...
        let partition_by = if dialect_of!(self is BigQueryDialect | GenericDialect)
    ...
    pub fn parse_alter_table_operation(&mut self) -> Result<AlterTableOperation, ParserError> {
            ...
            && self.parse_keyword(Keyword::UNFREEZE)
        {
            ...
            if !options.is_empty() {
                ...
                options = self.parse_options_with_keywords(&[Keyword::SET, Keyword::OPTIONS])?;
                if !options.is_empty() {
                    AlterTableOperation::SetTblProperties {
                        table_properties: options,
                    }
    ...
    fn parse_alter_function(&mut self) -> Result<Statement, ParserError> {
        ...
        let options = self.parse_comma_separated(Parser::parse_sql_option)?;
        self.expect_token(&Token::RParen)?;
        Ok(Statement::AlterFunction { name, options })
    }
    ...
    fn parse_alter_procedure(&mut self) -> Result<Statement, ParserError> {
        ...
        let options = self.parse_comma_separated(Parser::parse_sql_option)?;
        self.expect_token(&Token::RParen)?;
        Ok(Statement::AlterProcedure { name, options })
    }
        ...
        loop {
            if self.parse_keywords(&[Keyword::SET, Keyword::OPTIONS]) {
                ...
                operations.push(AlterViewOperation::AlterColumn {
                    column_name,
                    operation: AlterColumnOperation::SetOptions {
                        options: col_options,
                    },
                });
            } else {
    ...
    fn parse_data_type_helper(
        ...
        let mut data = match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::RANGE_DATETIME => Ok(DataType::Range(Box::new(DataType::Datetime(None)))),
                Keyword::RANGE_TIMESTAMP => Ok(DataType::Range(Box::new(DataType::Timestamp(
                    None,
                    TimezoneInfo::None,
                )))),
                Keyword::RANGE => {
