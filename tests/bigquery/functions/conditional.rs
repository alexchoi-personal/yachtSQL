...
async fn test_ifnull() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT IFNULL(NULL, 10)")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_ifnull_not_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT IFNULL(5, 10)").await.unwrap();
    assert_table_eq!(result, [[5]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_ifnull_string() {
    let session = create_session();
    let result = session
...
#[tokio::test(flavor = "current_thread")]
async fn test_zeroifnull_not_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT ZEROIFNULL(42)").await.unwrap();
...
async fn test_nvl() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NVL(NULL, 'default')")
        .await
        .unwrap();
    assert_table_eq!(result, [["default"]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_nvl_not_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NVL('value', 'default')")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_nvl2_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NVL2(NULL, 'not_null', 'is_null')")
        .await
        .unwrap();
    assert_table_eq!(result, [["is_null"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_nvl2_not_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NVL2('value', 'not_null', 'is_null')")
        .await
        .unwrap();
    assert_table_eq!(result, [["not_null"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_nvl2_integer() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NVL2(5, 100, 200)")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_if_with_null_condition() {
    let session = create_session();
    let result = session
...
#[tokio::test(flavor = "current_thread")]
async fn test_if_with_numeric_result() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT IF(1 > 0, 100, 200)")
        .await
        .unwrap();
    assert_table_eq!(result, [[100]]);
}
