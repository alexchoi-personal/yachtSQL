...
#[tokio::test(flavor = "current_thread")]
async fn test_array_first_fn() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT ARRAY_FIRST([1, 2, 3])")
        .await
        .unwrap();
    assert_table_eq!(result, [[1]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_array_first_empty() {
    let session = create_session();
    let result = session.execute_sql("SELECT ARRAY_FIRST([])").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_array_last() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT ARRAY_LAST([1, 2, 3])")
        .await
        .unwrap();
    assert_table_eq!(result, [[3]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_array_last_empty() {
    let session = create_session();
    let result = session.execute_sql("SELECT ARRAY_LAST([])").await.unwrap();
    assert_table_eq!(result, [[null]]);
}
