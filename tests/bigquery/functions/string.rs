...
async fn test_left_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT LEFT(NULL, 3)").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_left_null_length() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LEFT('hello', NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_left_utf8_chars() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LEFT('абвгд', 3)")
        .await
        .unwrap();
    assert_table_eq!(result, [["абв"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_right_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT RIGHT(NULL, 3)").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_right_null_length() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT RIGHT('hello', NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_right_utf8_chars() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT RIGHT('абвгд', 3)")
        .await
        .unwrap();
    assert_table_eq!(result, [["вгд"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_split_null_value() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SPLIT(NULL, ',')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_split_long_delimiter() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SPLIT('a:::b:::c', ':::')")
        .await
...
#[tokio::test(flavor = "current_thread")]
async fn test_split_with_empty_parts() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SPLIT('a,,b', ',')")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_to_code_points_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT TO_CODE_POINTS(NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_code_points_to_string_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT CODE_POINTS_TO_STRING(NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_code_points_to_bytes_empty() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT CODE_POINTS_TO_BYTES([])")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_chr_unicode() {
    let session = create_session();
    let result = session.execute_sql("SELECT CHR(128512)").await.unwrap();
    assert_table_eq!(result, [["\u{1F600}"]]);
}
...
async fn test_unicode_emoji() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT UNICODE('\u{1F600}')")
        .await
        .unwrap();
    assert_table_eq!(result, [[128512]]);
}
...
async fn test_soundex_empty() {
    let session = create_session();
    let result = session.execute_sql("SELECT SOUNDEX('')").await.unwrap();
    assert_table_eq!(result, [[""]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_soundex_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT SOUNDEX(NULL)").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_soundex_single_letter() {
    let session = create_session();
    let result = session.execute_sql("SELECT SOUNDEX('A')").await.unwrap();
    assert_table_eq!(result, [["A000"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_soundex_with_h_w() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SOUNDEX('Ashworth')")
        .await
        .unwrap();
    assert_table_eq!(result, [["A263"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_edit_distance_identical() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT EDIT_DISTANCE('hello', 'hello')")
        .await
        .unwrap();
    assert_table_eq!(result, [[0]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_edit_distance_null_first() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT EDIT_DISTANCE(NULL, 'hello')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_edit_distance_null_second() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT EDIT_DISTANCE('hello', NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_edit_distance_empty() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT EDIT_DISTANCE('', 'abc')")
        .await
        .unwrap();
...
async fn test_normalize_nfd() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LENGTH(NORMALIZE('\\u00ea', NFD))")
        .await
        .unwrap();
    assert_table_eq!(result, [[2]]);
}
...
async fn test_normalize_nfkd() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NORMALIZE('\\u2168', NFKD)")
        .await
        .unwrap();
    assert_table_eq!(result, [["IX"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_normalize_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT NORMALIZE(NULL)").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_normalize_and_casefold_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT NORMALIZE_AND_CASEFOLD(NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_format_hex_lower() {
    let session = create_session();
    let result = session
...
async fn test_format_octal() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT FORMAT('%o', 64)")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_format_scientific_lower() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT FORMAT('%e', 1234.5)")
        .await
        .unwrap();
    assert_table_eq!(result, [["1.234500e+03"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_format_with_precision() {
    let session = create_session();
    let result = session
...
async fn test_format_null() {
    let session = create_session();
    let result = session.execute_sql("SELECT FORMAT(NULL)").await.unwrap();
    assert_table_eq!(result, [[null]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_substr_zero_position() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SUBSTR('hello', 0, 3)")
        .await
        .unwrap();
    assert_table_eq!(result, [["hel"]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_substr_utf8() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SUBSTR('абвгд', 2, 2)")
        .await
        .unwrap();
    assert_table_eq!(result, [["бв"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_substr_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT SUBSTR(NULL, 1, 3)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_translate_null_source() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT TRANSLATE(NULL, 'abc', 'xyz')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_translate_null_from() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT TRANSLATE('hello', NULL, 'xyz')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_translate_null_to() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT TRANSLATE('hello', 'abc', NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
async fn test_replace_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT REPLACE(NULL, 'a', 'b')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_reverse_utf8() {
    let session = create_session();
    let result = session.execute_sql("SELECT REVERSE('абв')").await.unwrap();
    assert_table_eq!(result, [["вба"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_reverse_array() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT REVERSE([1, 2, 3])")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_instr_empty_substr() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT INSTR('hello', '')")
        .await
        .unwrap();
    assert_table_eq!(result, [[0]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_instr_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT INSTR(NULL, 'a')")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_strpos_bytes() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT STRPOS('hello', 'l')")
        .await
        .unwrap();
    assert_table_eq!(result, [[3]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_contains_substr_with_array() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT CONTAINS_SUBSTR(['hello', 'world'], 'world')")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_contains_substr_with_numbers() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT CONTAINS_SUBSTR(12345, '234')")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_lpad_empty_pattern() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LPAD('ab', 5, '')")
        .await
        .unwrap();
    assert_table_eq!(result, [["ab"]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_rpad_empty_pattern() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT RPAD('ab', 5, '')")
        .await
        .unwrap();
    assert_table_eq!(result, [["ab"]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_lpad_utf8() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LPAD('а', 5, 'б')")
        .await
        .unwrap();
    assert_table_eq!(result, [["бббба"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_ascii_utf8() {
    let session = create_session();
    let result = session.execute_sql("SELECT ASCII('абв')").await.unwrap();
    assert_table_eq!(result, [[1072]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_byte_length_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT BYTE_LENGTH(NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_char_length_null() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT CHAR_LENGTH(NULL)")
        .await
        .unwrap();
    assert_table_eq!(result, [[null]]);
}
...
async fn test_length_array() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT LENGTH([1, 2, 3])")
        .await
        .unwrap();
    assert_table_eq!(result, [[3]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_initcap_all_caps() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT INITCAP('HELLO WORLD')")
        .await
...
#[tokio::test(flavor = "current_thread")]
async fn test_initcap_numbers() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT INITCAP('hello123world456')")
        .await
        .unwrap();
    assert_table_eq!(result, [["Hello123world456"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_ends_with_empty() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT ENDS_WITH('hello', '')")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_starts_with_empty() {
    let session = create_session();
    let result = session
        .execute_sql("SELECT STARTS_WITH('hello', '')")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_concat_empty() {
    let session = create_session();
    let result = session.execute_sql("SELECT CONCAT()").await.unwrap();
    assert_table_eq!(result, [[""]]);
}
