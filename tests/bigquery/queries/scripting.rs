...
async fn test_assert_success() {
    ...
    session.execute_sql("ASSERT TRUE").await.unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_assert_with_expression() {
    let session = create_session();
    ...
    session
        ...
        .await
        .unwrap();
    session.execute_sql("ASSERT x > 5").await.unwrap();
...
async fn test_assert_with_message() {
    ...
    let result = session
        .execute_sql("ASSERT 1 = 2 AS 'One does not equal two'")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("One does not equal two"));
}
...
async fn test_assert_with_subquery() {
    ...
    session
        .execute_sql("ASSERT (SELECT COUNT(*) FROM nums) = 3")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_raise_without_message() {
    let session = create_session();

    let result = session.execute_sql("RAISE").await;
    assert!(result.is_err());
}

#[tokio::test(flavor = "current_thread")]
async fn test_raise_expression() {
    let session = create_session();

    let result = session.execute_sql("RAISE 'Direct error message'").await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("Direct error message"));
}

#[tokio::test(flavor = "current_thread")]
async fn test_create_procedure_basic() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE add_one(x INT64)
            BEGIN
                SELECT x + 1;
            END",
        )
    ...
    let result = session.execute_sql("CALL add_one(5)").await.unwrap();
    assert_table_eq!(result, [[6]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_create_procedure_with_out_param() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE double_val(IN x INT64, OUT y INT64)
            BEGIN
                SET y = x * 2;
            END",
        )
    ...
    session.execute_sql("CALL double_val(5, 0)").await.unwrap();
    let result = session.execute_sql("SELECT 1").await.unwrap();
    assert_table_eq!(result, [[1]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_create_procedure_with_inout_param() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE increment(INOUT val INT64)
            BEGIN
                SET val = val + 1;
            END",
        )
    ...
    session.execute_sql("CALL increment(10)").await.unwrap();
    let result = session.execute_sql("SELECT 1").await.unwrap();
    assert_table_eq!(result, [[1]]);
}
...
async fn test_create_or_replace_procedure() {
    ...
    session
        .execute_sql(
            "CREATE OR REPLACE PROCEDURE get_val()
            BEGIN
                SELECT 2;
            END",
        )
    ...
    let result = session.execute_sql("CALL get_val()").await.unwrap();
    assert_table_eq!(result, [[2]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_create_procedure_if_not_exists() {
    let session = create_session();
    ...
    let result = session.execute_sql("CALL my_proc()").await.unwrap();
    assert_table_eq!(result, [[1]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_procedure_with_multiple_params() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE compute(a INT64, b INT64, c INT64)
            BEGIN
                SELECT a * b + c;
            END",
        )
    ...
    let result = session.execute_sql("CALL compute(2, 3, 4)").await.unwrap();
    assert_table_eq!(result, [[10]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_procedure_with_no_params() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE return_constant()
            BEGIN
                SELECT 42;
            END",
        )
    ...
    let result = session.execute_sql("CALL return_constant()").await.unwrap();
    assert_table_eq!(result, [[42]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_labeled_while_loop() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_set_from_compound_identifier() {
    let session = create_session();

    session
        .execute_sql("DECLARE person STRUCT<name STRING, age INT64> DEFAULT ('Bob', 25)")
        .await
        .unwrap();

    session
        .execute_sql("DECLARE target_name STRING")
        .await
        .unwrap();
    session
        .execute_sql("SET target_name = person.name")
        .await
        .unwrap();

    let result = session.execute_sql("SELECT target_name").await.unwrap();
    assert_table_eq!(result, [["Bob"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_set_system_variable_with_at() {
    let session = create_session();

    session
        .execute_sql("SET @@time_zone = 'UTC'")
        .await
        .unwrap();

    session.execute_sql("DECLARE tz STRING").await.unwrap();
    session.execute_sql("SET tz = @@time_zone").await.unwrap();

    let result = session.execute_sql("SELECT tz").await.unwrap();
    assert_table_eq!(result, [["UTC"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_set_with_null_value() {
    let session = create_session();
    ...
    session.execute_sql("SET x = NULL").await.unwrap();

    let result = session.execute_sql("SELECT x IS NULL").await.unwrap();
    assert_table_eq!(result, [[true]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_execute_immediate_empty_error() {
    let session = create_session();

    session.execute_sql("DECLARE y INT64").await.unwrap();

    session
        .execute_sql("EXECUTE IMMEDIATE 'SELECT 42' INTO y")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_case_statement_without_else() {
    let session = create_session();
    ...
    session
        ...
        .unwrap();
    session
        .execute_sql("DECLARE result STRING DEFAULT 'unchanged'")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_case_statement_no_match_no_else() {
    let session = create_session();

    session
        .execute_sql("DECLARE x INT64 DEFAULT 99")
        .await
        .unwrap();
    session
        .execute_sql("DECLARE result STRING DEFAULT 'unchanged'")
    ...
    let result = session.execute_sql("SELECT result").await.unwrap();
    assert_table_eq!(result, [["unchanged"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_for_loop_empty_result() {
    let session = create_session();

    session
        .execute_sql("DECLARE count INT64 DEFAULT 0")
        .await
        .unwrap();

    session
        .execute_sql(
            "FOR row IN (SELECT val FROM UNNEST(CAST([] AS ARRAY<INT64>)) AS val)
            DO
                SET count = count + 1;
            END FOR",
        )
    ...
    let result = session.execute_sql("SELECT count").await.unwrap();
    assert_table_eq!(result, [[0]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_loop_with_label() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_if_with_begin_end_block() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_set_parenthesized_single_variable() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_multiple_declare_statements_same_type() {
    let session = create_session();

    session
        .execute_sql("DECLARE a, b, c, d INT64 DEFAULT 1")
        .await
        .unwrap();

    let result = session.execute_sql("SELECT a + b + c + d").await.unwrap();
    assert_table_eq!(result, [[4]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_declare_without_type() {
    let session = create_session();

    session.execute_sql("DECLARE x DEFAULT 123").await.unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_set_value_from_at_variable() {
    let session = create_session();

    session
        .execute_sql("SET @@time_zone = 'America/New_York'")
        .await
        .unwrap();
    ...
    let result = session.execute_sql("SELECT tz").await.unwrap();
    assert_table_eq!(result, [["America/New_York"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_set_value_from_struct_field() {
    let session = create_session();

    session
        .execute_sql("DECLARE s STRUCT<x INT64, y INT64> DEFAULT (10, 20)")
        .await
        .unwrap();

    session.execute_sql("DECLARE result INT64").await.unwrap();
    session.execute_sql("SET result = s.x + s.y").await.unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_set_identifier_as_string_literal() {
    let session = create_session();

    session.execute_sql("DECLARE mode STRING").await.unwrap();
    session.execute_sql("SET mode = test_mode").await.unwrap();

    let result = session.execute_sql("SELECT mode").await.unwrap();
    assert_table_eq!(result, [["test_mode"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_call_procedure_with_expression_args() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE sum_vals(a INT64, b INT64, c INT64)
            BEGIN
                SELECT a + b + c;
            END",
        )
    ...
    let result = session
        .execute_sql("CALL sum_vals(1 + 2, 3 * 2, 10 - 5)")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_procedure_or_replace_prefix() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE __orp__my_proc()
            BEGIN
                SELECT 999;
            END",
        )
...
#[tokio::test(flavor = "current_thread")]
async fn test_if_with_multiple_elseif_blocks() {
    let session = create_session();

    session
        .execute_sql("DECLARE x INT64 DEFAULT 25")
        .await
        .unwrap();
    session.execute_sql("DECLARE grade STRING").await.unwrap();
    ...
    let result = session.execute_sql("SELECT grade").await.unwrap();
    assert_table_eq!(result, [["F"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_case_with_many_when_blocks() {
    let session = create_session();

    session
        .execute_sql("DECLARE day_num INT64 DEFAULT 3")
        .await
        .unwrap();
    session
        .execute_sql("DECLARE day_name STRING")
        .await
        .unwrap();
    ...
    let result = session.execute_sql("SELECT day_name").await.unwrap();
    assert_table_eq!(result, [["Wednesday"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_while_with_label() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_assert_with_complex_condition() {
    let session = create_session();

    session
        .execute_sql("CREATE TABLE test_data (id INT64, value INT64)")
        .await
        .unwrap();
    session
        .execute_sql("INSERT INTO test_data VALUES (1, 100), (2, 200), (3, 300)")
        .await
        .unwrap();

    session
        .execute_sql("ASSERT (SELECT SUM(value) FROM test_data) > 500")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_execute_immediate_with_concat() {
    let session = create_session();

    session
        .execute_sql("DECLARE table_name STRING DEFAULT 'my_table'")
        .await
        .unwrap();

    session
        .execute_sql("EXECUTE IMMEDIATE 'CREATE TABLE ' || table_name || ' (id INT64)'")
        .await
        .unwrap();

    session
        .execute_sql("INSERT INTO my_table VALUES (1)")
        .await
        .unwrap();

    let result = session
        .execute_sql("SELECT id FROM my_table")
        .await
        .unwrap();
    assert_table_eq!(result, [[1]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_loop_without_label() {
    let session = create_session();
...
async fn test_for_loop_with_continue() {
    ...
    session
        .execute_sql(
            "FOR row IN (SELECT n FROM UNNEST([1, 2, 3, 4, 5]) AS n)
            DO
                IF row.n = 3 THEN
                    CONTINUE;
                END IF;
...
#[tokio::test(flavor = "current_thread")]
async fn test_single_declare_variable() {
    let session = create_session();

    session
        .execute_sql("DECLARE single_var INT64")
        .await
        .unwrap();
    session.execute_sql("SET single_var = 42").await.unwrap();

    let result = session.execute_sql("SELECT single_var").await.unwrap();
    assert_table_eq!(result, [[42]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_declare_with_expression_assignment() {
    let session = create_session();

    session
        .execute_sql("DECLARE calculated INT64 DEFAULT 5 * 10 + 3")
        .await
        .unwrap();

    let result = session.execute_sql("SELECT calculated").await.unwrap();
    assert_table_eq!(result, [[53]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_if_only_then_branch() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_case_searched_no_match() {
    let session = create_session();
    ...
    session
        .execute_sql(
            "CASE
                WHEN x > 10 THEN SET result = 'big';
                WHEN x > 5 THEN SET result = 'medium';
            END CASE",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_procedure_without_params_clause() {
    let session = create_session();

    session
        .execute_sql(
            "CREATE PROCEDURE no_params_proc()
            BEGIN
                SELECT 'no params';
            END",
        )
    ...
    let result = session.execute_sql("CALL no_params_proc()").await.unwrap();
    assert_table_eq!(result, [["no params"]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_repeat_with_multiple_statements() {
    let session = create_session();
    ...
    session
        ...
        .unwrap();
    session
        .execute_sql("DECLARE product INT64 DEFAULT 1")
        .await
        .unwrap();
    ...
    let result = session.execute_sql("SELECT sum, product").await.unwrap();
    assert_table_eq!(result, [[10, 24]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_set_with_case_expression() {
    let session = create_session();
    ...
    session
        .execute_sql("SET result = CASE x WHEN 1 THEN 'one' WHEN 2 THEN 'two' ELSE 'other' END")
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_execute_immediate_into_multiple() {
    let session = create_session();
    ...
    session
        .execute_sql("EXECUTE IMMEDIATE 'SELECT 42, \"hello\"' INTO a, b")
        .await
        .unwrap();
