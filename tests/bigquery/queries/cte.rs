...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_column_aliases() {
    let session = create_session();
    setup_tables(&session).await;

    let result = session
        .execute_sql(
            "WITH RECURSIVE emp_tree (emp_id, emp_name, mgr_id, lvl) AS (
            ...
            SELECT emp_name, lvl FROM emp_tree ORDER BY lvl, emp_id",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_anchor_in_nested_union() {
    let session = create_session();
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE nums AS (
                ...
                SELECT 10 AS n
                UNION ALL
                SELECT n + 1 FROM nums WHERE n < 3 OR (n >= 10 AND n < 12)
            )
    ...
    assert_table_eq!(result, [[1], [2], [3], [10], [11], [12]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_join_in_anchor() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            ...
            SELECT name, depth FROM hierarchy ORDER BY depth, id",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_derived_table() {
    let session = create_session();

    let result = session
        .execute_sql(
            "WITH RECURSIVE cnt AS (
                SELECT n FROM (SELECT 1 AS n) sub
                UNION ALL
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_select_union_anchor() {
    let session = create_session();
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE cnt AS (
                ...
                UNION ALL
                SELECT n + 10 FROM cnt WHERE n < 3
            )
            SELECT n FROM cnt ORDER BY n",
        )
        .await
    ...
    assert_table_eq!(result, [[1], [2], [11], [12]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_subquery_reference() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE emp_levels AS (
                SELECT * FROM (SELECT id, name, manager_id, 1 AS lvl FROM employees WHERE manager_id IS NULL) anchor
                UNION ALL
                SELECT e.id, e.name, e.manager_id, el.lvl + 1
                FROM employees e
                JOIN emp_levels el ON e.manager_id = el.id
            )
            SELECT name, lvl FROM emp_levels ORDER BY lvl, id",
        ).await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_left_references_cte() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_cte_with_values_clause() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_cte_with_subquery_body() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            ...
            SELECT name FROM wrapper ORDER BY salary DESC",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_values_anchor() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_derived_table_reference() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE tree AS (
                ...
                FROM (SELECT * FROM employees WHERE manager_id IS NULL) anchors
                UNION ALL
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_unnest_in_recursive_part() {
    let session = create_session();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_anchor_with_set_operation() {
    let session = create_session();
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE combined AS (
                SELECT 1 AS n UNION SELECT 2 AS n
                UNION ALL
                SELECT n + 10 FROM combined WHERE n < 5
            )
            SELECT n FROM combined ORDER BY n",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_join_referencing_cte() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE emp_chain AS (
                ...
                FROM employees e
                INNER JOIN emp_chain c ON e.manager_id = c.id
            )
            SELECT name, level FROM emp_chain ORDER BY level, id",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_with_deeply_nested_union() {
    let session = create_session();
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE deep AS (
                ...
                UNION ALL
                SELECT n + 1 FROM deep WHERE (n < 3) OR (n >= 100 AND n < 102)
            )
            SELECT n FROM deep ORDER BY n",
        )
        .await
        .unwrap();

    assert_table_eq!(result, [[1], [2], [3], [100], [101], [102]]);
}

#[tokio::test(flavor = "current_thread")]
async fn test_recursive_cte_left_side_references_cte_in_subquery() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            "WITH RECURSIVE tree AS (
                ...
                FROM employees e
                JOIN (SELECT * FROM tree) t ON e.manager_id = t.id
            )
            SELECT name, lvl FROM tree ORDER BY lvl, id",
        )
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn test_cte_with_set_operation_in_body() {
    let session = create_session();
    setup_tables(&session).await;
    ...
    let result = session
        .execute_sql(
            "WITH combined AS (
                SELECT name, salary FROM employees WHERE salary > 150000
                UNION ALL
                SELECT name, salary FROM employees WHERE salary < 90000
            )
            SELECT name FROM combined ORDER BY salary",
        )
        .await
        .unwrap();

    assert_table_eq!(result, [["Developer"], ["CEO"]]);
}
