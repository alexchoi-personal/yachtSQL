...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_without_unit() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT COUNT(*) <= 100 FROM large_data TABLESAMPLE SYSTEM (50)")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_zero_rows() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT COUNT(*) FROM large_data TABLESAMPLE SYSTEM (0 ROWS)")
        .await
        .unwrap();
    assert_table_eq!(result, [[0]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_after_alias_with_explicit_as() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT COUNT(*) <= 100 FROM large_data AS ld TABLESAMPLE SYSTEM (50 PERCENT)")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_after_alias_without_as() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT COUNT(*) <= 100 FROM large_data ld TABLESAMPLE SYSTEM (50 PERCENT)")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_with_specific_rows() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT COUNT(*) <= 5 FROM large_data TABLESAMPLE SYSTEM (5 ROWS)")
        .await
        .unwrap();
    assert_table_eq!(result, [[true]]);
}
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_invalid_float_value() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT * FROM large_data TABLESAMPLE SYSTEM (1.5 PERCENT)")
        .await;
    assert!(result.is_err());
}

#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_invalid_string_value() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT * FROM large_data TABLESAMPLE SYSTEM ('fifty' PERCENT)")
        .await;
    assert!(result.is_err());
}

#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_invalid_expression_value() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT * FROM large_data TABLESAMPLE SYSTEM (10 + 5 PERCENT)")
        .await;
    assert!(result.is_err());
}

#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_with_cte() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql(
            "WITH sampled AS (SELECT * FROM large_data TABLESAMPLE SYSTEM (100 PERCENT))
...
#[tokio::test(flavor = "current_thread")]
async fn test_tablesample_negative_value() {
    let session = create_session();
    setup_large_table(&session).await;

    let result = session
        .execute_sql("SELECT * FROM large_data TABLESAMPLE SYSTEM (-10 PERCENT)")
        .await;
    assert!(result.is_err() || result.unwrap().num_rows() == 0);
}
