...
async fn main() -> yachtsql::Result<()> {
    ...
    let results = session.execute_sql("SELECT * FROM users").await?;
    println!("{:?}", results);
    Ok(())
}
```
...
session1.execute_sql("CREATE TABLE foo (id INT64)").await?;
session1.execute_sql("INSERT INTO foo VALUES (1)").await?;
